...
# ============================================================================
# Request Models — Model yêu cầu từ client
# ============================================================================

class SaveHistoryRequest(BaseModel):
    """Request model for saving execution history — lưu lịch sử chạy"""
    execution_id: str           # ID duy nhất
    algorithm_name: str         # tên thuật toán
    file_id: str                # ID file dữ liệu
    parameters: Dict[str, Any]  # tham số
    results: Dict[str, Any]     # kết quả
    execution_time: float       # thời gian chạy
    analysis: Optional[Dict[str, Any]] = None  # phân tích kết quả
    status: str = "success"     # trạng thái
...
def save_execution_history(
    ...
    """
    Save execution history to disk — lưu lịch sử ra file JSON
    ...
    timestamp = datetime.now().isoformat()  # thời điểm lưu
...
async def save_history(request: SaveHistoryRequest):
    """
    Save algorithm execution history — API endpoint lưu lịch sử
...
async def get_all_histories():
    """
    Get all execution histories — lấy tất cả lịch sử (mới nhất trước)
...
async def get_history(execution_id: str):
    """
    Get specific execution history by ID — lấy lịch sử theo ID
...
async def get_histories_by_algorithm(algorithm_name: str):
    """
    Get all histories for a specific algorithm — lấy lịch sử theo thuật toán
...
async def delete_history(execution_id: str):
    """
    Delete an execution history record — xóa lịch sử
...
def analyze_results(
    ...
    """
    Tự động phân tích kết quả thuật toán
    ...
    # Lấy accuracy hoặc metric chính
...
async def analyze_execution(execution_id: str):
    """
    Phân tích tự động kết quả chạy và lưu phân tích
    ...
    try:
        ...
        # Perform analysis — chạy phân tích
        analysis = analyze_results(history.algorithm_name, history.results)
        
        # Update history with analysis — cập nhật phân tích vào lịch sử
        history.analysis = analysis.dict()
        
        # Save updated history — lưu lại
        with open(history_file, "w") as f:
            json.dump(history.dict(), f, indent=2)
